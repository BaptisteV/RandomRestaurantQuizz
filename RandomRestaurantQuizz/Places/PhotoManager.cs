...
public class PhotoManager
{
    ...
    public PhotoManager(HttpClient httpClient, string apiKey, ILogger<PhotoManager> logger)
    {
            ...
            .Select(async (place, i) =>
            {
                try
                {
                    ...
                    for (var p = 0; p < place.Photos!.Count; p++)
                    {
                        ...
                        {
                            var image = await File.ReadAllBytesAsync(GetFilename(place, p), cancellationToken);
                            place.Photos[p].DownloadedImage = image;
                        }
                    }
    ...
    {
        var filename = GetFilename(place, photoIndex);
        ...
        var writes = place.Photos!
            .Select(async (ph, i) =>
            {
                try
                {
                    ...
                    {
                        var tempFileName = GetFilename(place, i);
